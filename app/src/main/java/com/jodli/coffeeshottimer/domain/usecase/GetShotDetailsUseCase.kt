...
class GetShotDetailsUseCase @Inject constructor(
    ...
    suspend fun getShotDetails(shotId: String): Result<ShotDetails> {
        return try {
            ...
            if (shotResult.isFailure) {
                return Result.failure(shotResult.exceptionOrNull() ?: Exception("Failed to get shot"))
            }
            ...
            if (beanResult.isFailure) {
                return Result.failure(beanResult.exceptionOrNull() ?: Exception("Failed to get bean"))
            }
            ...
            val nextShot = if (currentIndex < sortedShots.size - 1) sortedShots[currentIndex + 1] else null
    ...
    suspend fun compareShotsDetails(shotId1: String, shotId2: String): Result<ShotComparison> {
        return try {
            ...
            if (shot1Result.isFailure) {
                return Result.failure(shot1Result.exceptionOrNull() ?: Exception("Failed to get first shot"))
            }
            if (shot2Result.isFailure) {
                return Result.failure(shot2Result.exceptionOrNull() ?: Exception("Failed to get second shot"))
            }
    ...
    suspend fun getLastShotForBean(beanId: String): Result<ShotDetails?> {
        return try {
            ...
            if (lastShotResult.isFailure) {
                return Result.failure(lastShotResult.exceptionOrNull() ?: Exception("Failed to get last shot"))
            }
     ...
     */
    private fun calculateShotAnalysis(shot: Shot, relatedShots: List<Shot>, bean: Bean): ShotAnalysis {
...
data class ShotComparison(
    ...
    fun getFormattedDifferences(): Map<String, String> {
        return mapOf(
            "weightIn" to "${if (weightInDifference >= 0) "+" else ""}${String.format("%.1f", weightInDifference)}g",
            "weightOut" to "${if (weightOutDifference >= 0) "+" else ""}${String.format("%.1f", weightOutDifference)}g",
            "extractionTime" to "${if (extractionTimeDifference >= 0) "+" else ""}${extractionTimeDifference}s",
            "brewRatio" to "${if (brewRatioDifference >= 0) "+" else ""}${String.format("%.2f", brewRatioDifference)}"
        )
    }
}