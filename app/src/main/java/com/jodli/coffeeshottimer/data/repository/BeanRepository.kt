...
class BeanRepository @Inject constructor(
    ...
    suspend fun updateBeanActiveStatus(beanId: String, isActive: Boolean): Result<Unit> {
        return try {
            ...
            Result.failure(
                RepositoryException.DatabaseError(
                    "Failed to update bean active status",
                    exception
                )
            )
        }
     ...
     */
    fun getFilteredBeans(activeOnly: Boolean, searchQuery: String): Flow<Result<List<Bean>>> =
        flow {
    ...
    suspend fun getActiveBeanCount(): Result<Int> {
        return try {
            ...
            Result.failure(
                RepositoryException.DatabaseError(
                    "Failed to get active bean count",
                    exception
                )
            )
        }
