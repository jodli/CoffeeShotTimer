...
class TypographyAndSpacingTest {
    ...
    fun `screen titles use headlineMedium with correct font weight`() {
        ...
        assertTrue("Screen title should use bold-style font weight", 
    ...
    fun `typography hierarchy maintains proper size relationships`() {
        ...
        assertTrue("Display large should be larger than headline large",
            testTypography.displayLarge.fontSize > testTypography.headlineLarge.fontSize)
        assertTrue("Display medium should be larger than headline medium",
            testTypography.displayMedium.fontSize > testTypography.headlineMedium.fontSize)
        assertTrue("Display small should be larger than headline small",
        ...
        assertTrue("Headline large should be larger than title large",
            testTypography.headlineLarge.fontSize > testTypography.titleLarge.fontSize)
        assertTrue("Headline medium should be larger than title medium",
            testTypography.headlineMedium.fontSize > testTypography.titleMedium.fontSize)
        assertTrue("Headline small should be larger than title small",
    ...
    fun `screen padding is consistently applied`() {
        ...
        assertEquals("Screen padding should match medium spacing", 
            testSpacing.medium, testSpacing.screenPadding)
    }
    ...
    fun `card spacing follows consistent patterns`() {
        ...
        assertEquals("Card padding should match medium spacing", 
            testSpacing.medium, testSpacing.cardPadding)
    }
    ...
    fun `accessibility requirements are met through spacing`() {
        ...
        assertTrue("Touch target should meet accessibility minimum (44.dp)",
            testSpacing.touchTarget.value >= 44f)
        
        // Verify touch target is larger than other spacing values for usability
        assertTrue("Touch target should be larger than large spacing",
    ...
    fun `design system maintains visual hierarchy through typography and spacing`() {
        ...
        assertTrue("Screen titles should be larger than card titles",
            testTypography.headlineMedium.fontSize > testTypography.titleMedium.fontSize)
        assertTrue("Card titles should be larger than body text",
            testTypography.titleMedium.fontSize > testTypography.bodyMedium.fontSize)
        
        // Spacing hierarchy supports visual hierarchy
        assertTrue("Large spacing should create more separation than medium",
            testSpacing.large > testSpacing.medium)
        assertTrue("Card corners should be larger than button corners for prominence",
            testSpacing.cornerLarge > testSpacing.cornerMedium)
        
        // Component sizing supports hierarchy
        assertTrue("Medium icons should be larger than small icons for importance",
            testSpacing.iconMedium > testSpacing.iconSmall)
        assertTrue("Empty state icons should be largest for prominence",
    ...
    fun `typography font weights support content hierarchy`() {
        ...
        assertTrue("Display styles should use bold weights",
            testTypography.displayLarge.fontWeight?.weight ?: 0 >= FontWeight.Bold.weight)
        assertTrue("Display medium should use bold weight",
            testTypography.displayMedium.fontWeight?.weight ?: 0 >= FontWeight.Bold.weight)
        assertTrue("Display small should use bold weight",
            testTypography.displaySmall.fontWeight?.weight ?: 0 >= FontWeight.Bold.weight)
        
        // Headline styles use semi-bold weights
        assertEquals("Headlines should use SemiBold weight", 
            FontWeight.SemiBold, testTypography.headlineMedium.fontWeight)
        
        // Title styles use medium weights
        assertEquals("Titles should use Medium weight", 
            FontWeight.Medium, testTypography.titleMedium.fontWeight)
        
        // Body styles use normal weights
        assertEquals("Body text should use Normal weight", 
            FontWeight.Normal, testTypography.bodyMedium.fontWeight)
        
        // Label styles use medium weights for buttons
        assertEquals("Labels should use Medium weight", 
            FontWeight.Medium, testTypography.labelLarge.fontWeight)
    }
    ...
    fun `line heights provide appropriate text readability`() {
        ...
        assertTrue("Display large line height should be larger than font size",
            testTypography.displayLarge.lineHeight > testTypography.displayLarge.fontSize)
        assertTrue("Headline medium line height should be larger than font size",
            testTypography.headlineMedium.lineHeight > testTypography.headlineMedium.fontSize)
        assertTrue("Title medium line height should be larger than font size",
            testTypography.titleMedium.lineHeight > testTypography.titleMedium.fontSize)
        assertTrue("Body medium line height should be larger than font size",
        ...
        assertTrue("Headline line height ratio should be reasonable (1.2-1.6)", 
        ...
        assertTrue("Body line height ratio should be reasonable (1.2-1.6)", 
