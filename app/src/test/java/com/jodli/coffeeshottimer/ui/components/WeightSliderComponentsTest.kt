...
class WeightSliderComponentsTest {
    ...
    fun `weight slider should handle whole gram values correctly`() {
        ...
        testValues.forEachIndexed { index, value ->
            ...
            assertEquals(
                "Value $value should round to ${expectedValues[index]}",
    ...
    fun `weight slider should handle invalid input gracefully`() {
        ...
        invalidValues.forEach { value ->
            ...
            if (value == "" || value == "abc" || value == "18.5.5") {
                assertTrue(
                    "Value '$value' should parse to null or be handled gracefully",
        ...
        assertTrue(
            "Negative value should be handled by coercion in actual usage",
            negativeFloat != null
        )
    }
    ...
    fun `typical range validation should work correctly`() {
        ...
        assertTrue(
            "Value $inRangeValue should be in typical range",
            inRangeValue in typicalRangeStart..typicalRangeEnd
        )
        assertFalse(
            "Value $belowRangeValue should not be in typical range",
            belowRangeValue in typicalRangeStart..typicalRangeEnd
        )
        assertFalse(
            "Value $aboveRangeValue should not be in typical range",
